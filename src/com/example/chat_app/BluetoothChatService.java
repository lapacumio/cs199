...
public class BluetoothChatService {
    ...
    private static final String NAME = "Chat App";
    ...
    private ArrayList<UUID> usedUuids;
    ...
    public synchronized void start() {
    	...
    	if (!mConnThreads.isEmpty()) {
        	for(ConnectedThread mConnThread : mConnThreads){ mConnThread.cancel(); }
        	mConnThreads.clear();
        }
    ...
    public synchronized void connect(BluetoothDevice device) {
        ...
        for (UUID tempUuid : mUuids) {
    		if(!usedUuids.contains(tempUuid)){
	        	try {
	                mConnectThread = new ConnectThread(device, tempUuid);
    ...
    private class AcceptThread extends Thread {
        ...
        public void run() {
            ...
            try {
            	...
            	for (UUID tempUuid : mUuids) {
            		if(!usedUuids.contains(tempUuid)){
	            		serverSocket = mAdapter.listenUsingRfcommWithServiceRecord(NAME, tempUuid);
	                    socket = serverSocket.accept();
	                    if (socket != null) {
	                    	String address = socket.getRemoteDevice().getAddress();
	                    	if(mDeviceAddresses.contains(address)){ 
			                    ...
			                    usedUuids.add(tempUuid);
			                    Log.e(TAG, "flagged as used UUID: "+tempUuid.toString());
	                    	}
	                    }	                    
            		}
            	}
    ...
    private class ConnectThread extends Thread {
        ...
        public void run() {
            ...
            try {
            	...
            	//if (tempUuid.toString().contentEquals(mUuids.get(6).toString())) { // i don't get what this is for
                    connectionFailed();
            	//}
                // Close the socket
                try {
            ...
            //usedUuids.add(tempUuid);
            Log.e(TAG, "tried to connect through UUID: "+tempUuid.toString());
        }
